...
chrome.storage.local.get(['appleId', 'appPassword']).then(data => {
...
syncBtn.addEventListener('click', async () => {
  ...
  chrome.storage.local.set({ appleId, appPassword });
  ...
  try {
    ...
    const response = await chrome.tabs.sendMessage(tab.id, { type: 'GET_SCHEDULE' });
